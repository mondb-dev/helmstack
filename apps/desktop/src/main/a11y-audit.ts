...
/** A node from the CDP accessibility tree (`getFullAXTree` / `getPartialAXTree`). */
...
// ARIA roles that require specific state properties (CDP property name → roles).
...
/** Inputs to the pure report builder. */
export type A11yReportInput = {
  nodes: RawAxNode[];
  /** `<html lang>` value (ignored when `scoped`). */
  pageLang: string | null;
  /** `document.title` (ignored when `scoped`). */
  pageTitle: string;
  /** When true, page-level rules (lang, title) are skipped — the audit is a subtree. */
  scoped: boolean;
  /** Echoed back on the report when the audit was scoped to a selector. */
  selector?: string;
  url: string;
  tabId: TabId;
  capturedAt: number;
...
/**
 * Pure WCAG 2.2-aligned rule engine over a set of AX nodes. No CDP/DOM access,
 * so it is unit-tested directly. Page-level rules (3.1.1 lang, 2.4.2 title) are
 * applied only for a full-page audit (`scoped === false`).
 */
export function buildA11yReport(input: A11yReportInput): A11yAuditReport {
  const { nodes, pageLang, pageTitle, scoped } = input;

  const violations: A11yViolation[] = [];
  let passes = 0;

  const selectorFor = (node: RawAxNode, role: string): string =>
    node.backendDOMNodeId ? `[data-ax-node="${node.nodeId}"]` : `[role="${role}"]`;

  const push = (ruleId: string, node: RawAxNode, role: string, name: string | undefined, description: string, remediation: string) => {
  ...
  // ── DOM-level checks (page-wide; skipped for a scoped subtree audit) ──────
  ...
  if (scoped && nodes.length === 0) {
    recommendations.unshift(`Scoped audit: selector "${input.selector ?? ""}" matched no accessible elements (it may not exist, be hidden, or be aria-hidden).`);
  } else if (counts.critical === 0 && counts.serious === 0 && violations.length === 0) {
...
/**
 * Run the accessibility audit over CDP. Without `selector` the full page is
 * audited; with `selector` the audit is scoped to that element's AX subtree
 * (via `getPartialAXTree`) and page-level rules are skipped. Assumes the CDP
 * debugger is attached.
 */
export async function runA11yAudit(webContents: WebContents, tabId: TabId, selector?: string): Promise<A11yAuditReport> {
  await webContents.debugger.sendCommand("Accessibility.enable");

  if (selector) {
    const doc = await webContents.debugger.sendCommand("DOM.getDocument", { depth: 0 }) as { root: { nodeId: number } };
    const found = await webContents.debugger.sendCommand("DOM.querySelector", {
    ...
    let nodes: RawAxNode[] = [];
    if (found.nodeId) {
      const ax = await webContents.debugger.sendCommand("Accessibility.getPartialAXTree", {
        nodeId: found.nodeId,
    ...
    return buildA11yReport({
      nodes, pageLang: null, pageTitle: "", scoped: true, selector,
      url: webContents.getURL(), tabId, capturedAt: Date.now()
    });
  }

  const [axResult, domChecks] = await Promise.all([
    webContents.debugger.sendCommand("Accessibility.getFullAXTree", {}) as Promise<{ nodes: RawAxNode[] }>,
  ...
  return buildA11yReport({
    nodes: axResult.nodes,
    pageLang: domChecks.result.value.lang,
    pageTitle: domChecks.result.value.title,
    scoped: false,
